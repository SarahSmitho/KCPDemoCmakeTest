...
{	
	...
	//bzero(&send->addr, (unsigned long)sizeof(send->addr));
...
void loop(kcpObj *send)
{
	...
	while(1)
	{
		...
		if (sendNum < 500) {
			char temp[488] = {0};//与服务器初次通信	
			memset(temp,97,sizeof(temp));	
			int	ret = ikcp_send(send->pkcp,temp,(int)sizeof(temp)); 
			sendNum++;
			printf("ikcp_send发送连接请求： [%s] len=%d ret = %d sendNum = %d\n",temp,(int)sizeof(temp),ret,sendNum);//发送成功的
		}

		char buf[512]={0};

		//处理收消息
		n = recvfrom(send->sockfd,buf,512,0,(struct sockaddr *) &send->addr,&len);
