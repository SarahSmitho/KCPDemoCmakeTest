...
void loop(kcpObj *send)
{
	...
	while(1)
	{
		...
		if(n < 0)//检测是否有UDP数据包: kcp头部+data
			continue;
		recvUdpNum++;
		printf("UDP接收到数据包  大小= %d recvUdpNum= %d  \n",n,recvUdpNum);
		...
		// }
		recvNum++;
		printf("数据交互  ip = %s  port = %d recvNum = %d buf = %s\n",inet_ntoa(send->CientAddr.sin_addr),ntohs(send->CientAddr.sin_port),recvNum,buf);
...
//	memcpy(send.buff,Msg,sizeof(Msg));
	...
	ikcp_nodelay(kcp, 0, 1, 1, 0);//1, 10, 2, 1
